...
let SQLGrid = function(sequelizeOpts, bucketOptions) {
	...
	let dialect = isInstance
		? sequelizeOpts.options.dialect
		: sequelizeOpts.dialect;
	if (dialect === "sqlite") {
		defaultBucketOptions.concurrency = 1;
	} // Disable concurrency for sqlite
	...
	let fetchFileByRelation = async function(modeler, modeler_id) {
		let file = await files.findAll({
			where: {
				modeler,
				modeler_id
			},
		...
		if (!file.length) return Promise.reject(new Error("file not found"));
		return file[0].get({ plain: true });
	};
	...
	let fetchFileByName = async function(filename, revision = -1) {
		...
		if (revision === "all") {
			...
			if (fileList.length < revSteps + 1) {
				throw new Error("File revision does not exist!");
			}
		...
		let next = async function() {
			...
			while (queue.length < conf.concurrency && queueNum <= endNum) {
				...
				if (chunk != null) {
					queue.push({
						key: chunkKey,
						data: Promise.resolve(chunk),
						cached: true
					});
				} else {
					let query = Promise.resolve(
						(async function(fileId, num) {
							...
							return chunks.findById(fileChunk.chunk_id);
						})(file.id, queueNum)
					);
	...
	let createWriteStream = function(spec = {}) {
		...
		if (spec.filename == null) {
			throw new Error("filename must not be null");
		}
		...
		stream._write = async function(chunk, encoding, cb) {
			...
			if (file == null) {
				file = await files.create({
					...
					status: "Incomplete",
					modeler: spec.modeler,
					modeler_id: spec.modeler_id,
					mime: spec.mime
				});
			}
		...
		stream._final = async function(cb) {
			if (file) {
				...
				queue.push(Promise.resolve(writeChunk(file, num, currentChunk)));
	...
	let getFile = async function(spec = {}) {
		...
		if (
			spec.id == null &&
			spec.filename == null &&
			(spec.modeler == null && spec.modeler_id == null)
		) {
			throw new Error("filename or id required");
		}
		...
		} else if (spec.modeler && spec.modeler_id) {
			result = await fetchFileByRelation(spec.modeler, spec.modeler_id);
		} else {
			...
			if (Array.isArray(result)) {
				for (let file of result) {
					let stream = createReadStream({
						id: file.id,
						seekStart: spec.seekStart,
						seekEnd: spec.seekEnd
					});
	...
	let createReadStream = function(spec = {}) {
		...
		if (spec.id == null && spec.modeler == null && spec.modeler_id == null) {
		...
		stream._read = async function() {
			...
			if (file == null) {
				if (spec.id != null) {
					...
					file = await fetchFileByRelation(spec.modeler, spec.modeler_id);
				} else {
				...
				if (verifyHash) {
					...
					if (sha256 !== file.sha256) {
						process.nextTick(function() {
							stream.emit(
								"error",
								new Error("sha256 hash mismatch: File is likely corrupted!")
							);
						});
						return;
					}
